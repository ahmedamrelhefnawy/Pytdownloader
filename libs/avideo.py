...
class avideo:
    ...
    def __init__(self, yt_obj: YouTube):
        self.yt = yt_obj
        self.video_streams = yt_obj.streams.order_by("resolution")[::-1]
        self.audio_streams = yt_obj.streams.order_by("abr")[::-1]
    ...
    def ask_download(self):
        ...
        if user_input == 1:
            self.ask_download_video(self)
        else:
            self.ask_download_audio(self)
    ...
    def ask_download_video(self):
        ...
        # TODO: Get the correct stream by chosen_index
        self.download_video()
    ...
    def ask_download_audio(self):
        ...
        Place.ask_change_place()
        # TODO: Get the correct stream by user_input
        self.download_audio()
    ...
    def download_video(self, stream: Stream):
        ...
        if chosen_stream:
            if chosen_stream.is_adaptive:
                self.download_adaptive(self, stream)
            else:
            ...
            if closest_stream.is_adaptive:
                self.download_adaptive(self, closest_stream)
            else:
    ...
    def download_adaptive(self, stream: Stream):
        ...
        audio_name = f"audio.{self.yt.streams.subtype}"
    ...
    def print_video_streams(self):
        ...
        for stream_index in range(len(self.video_streams)):
            print(
                f"{stream_index + 1}: {Manipulate_stream.stream_info(self.video_streams[stream_index])}")

    def print_audio_streams(self):

        for stream_index in range(len(self.audio_streams)):
            print(
                f"{stream_index + 1}: {Manipulate_stream.stream_info(self.audio_streams[stream_index])}")
